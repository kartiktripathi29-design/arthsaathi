...
const C = { green: '#059669', ink: '#1E293B', sub: '#64748B', line: '#E2E8F0', bg: '#F8FAFC', soft: '#F0FDF4' }
        ...
        <div style={{ width: '100%', maxWidth: 760 }}>
          ...
          {(status === 'idle' || status === 'parsing' || status === 'error') && (
            <div style={{ textAlign: 'center', marginBottom: 28 }}>
              <h1 style={{ fontSize: 30, fontWeight: 800, color: C.ink, letterSpacing: '-0.03em', marginBottom: 8 }}>Decode your offer letter</h1>
              <p style={{ fontSize: 15, color: C.sub, lineHeight: 1.6, maxWidth: 540, margin: '0 auto' }}>
            ...
            <>
              <label
                ...
                style={{
                  ...
                  border: `2px dashed ${dragOver ? C.green : '#CBD5E1'}`, borderRadius: 14,
                  background: dragOver ? C.soft : C.bg, padding: '48px 24px', transition: 'all .15s',
                }}>
                <div style={{ fontSize: 34, marginBottom: 10 }}>📄</div>
                <div style={{ fontSize: 15, fontWeight: 600, color: C.ink, marginBottom: 4 }}>Drop your offer letter here, or click to browse</div>
                <div style={{ fontSize: 12.5, color: C.sub }}>PDF, Word (.docx) or image · password-protected PDFs supported</div>
              ...
              <p style={{ marginTop: 18, fontSize: 11.5, color: '#94A3B8', textAlign: 'center', lineHeight: 1.6 }}>
...
function Row({ label, amount, hint }: { label: string; amount: number; hint?: string }) {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '9px 0', borderBottom: `1px solid ${C.line}` }}>
      <span style={{ fontSize: 13, color: '#334155' }}>{label}{hint && <span style={{ color: '#94A3B8', fontWeight: 400 }}> · {hint}</span>}</span>
      <span style={{ fontSize: 13.5, fontWeight: 600, color: C.ink }}>{fmt(amount)}</span>
    </div>
  )
}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ background: '#fff', border: `1px solid ${C.line}`, borderRadius: 12, padding: '16px 18px', marginBottom: 14 }}>
      <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#94A3B8', margin: '0 0 8px' }}>{title}</p>
      {children}
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        <h1 style={{ fontSize: 24, fontWeight: 800, color: C.ink, letterSpacing: '-0.025em', margin: 0 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 14 }}>
        {[
          { k: 'Total CTC', v: total, c: C.ink },
          { k: 'Fixed (guaranteed)', v: fixed, c: C.green },
          { k: 'Variable (at-risk)', v: variable, c: '#B45309' },
        ].map(s => (
          <div key={s.k} style={{ background: C.bg, border: `1px solid ${C.line}`, borderRadius: 12, padding: '14px 16px' }}>
            <div style={{ fontSize: 11, color: C.sub, marginBottom: 4 }}>{s.k}</div>
      ...
      {/* Take-home estimate */}
      {monthlyInHand > 0 && (
        <div style={{ background: C.soft, border: '1px solid #A7F3D0', borderRadius: 12, padding: '16px 18px', marginBottom: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: 8 }}>
            <div>
              <div style={{ fontSize: 12, color: '#047857', fontWeight: 600 }}>Estimated monthly take-home</div>
              <div style={{ fontSize: 11, color: '#059669', marginTop: 2 }}>New regime · standard deduction only · before 80C/HRA</div>
            </div>
            <div style={{ fontSize: 26, fontWeight: 800, color: '#047857', letterSpacing: '-0.02em' }}>{fmt(monthlyInHand)}</div>
          </div>
          <p style={{ fontSize: 11, color: '#059669', margin: '10px 0 0', lineHeight: 1.5 }}>
      ...
      {d.notes && (
        <Section title="Notes from the offer">
          <p style={{ fontSize: 12.5, color: '#475569', lineHeight: 1.6, margin: 0 }}>{d.notes}</p>
        </Section>
      )}
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 18 }}>
        <button onClick={onReset} className="cta"
          style={{ padding: '12px 18px', background: '#fff', color: C.ink, border: `1px solid #CBD5E1`, borderRadius: 10, fontSize: 13.5, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
          ↺ Parse another
        </button>
        <Link href="/login" className="cta" style={{ flex: 1, minWidth: 220, textAlign: 'center', padding: '12px 18px', background: C.green, color: '#fff', borderRadius: 10, fontSize: 13.5, fontWeight: 700, textDecoration: 'none', fontFamily: 'inherit' }}>
