...
const INCOME_LABELS: Record<string, string> = {
  dividend: 'Dividend Income', fd: 'FD Interest', savings: 'Savings Bank Interest',
  other_int: 'Other Interest', gift_other: 'Gift from Non-Relatives',
  ltcg_eq: 'LTCG — Equity / MF', stcg_eq: 'STCG — Equity / MF',
  ltcg_prop: 'LTCG — Property', stcg_prop: 'STCG — Property',
  ltcg_other: 'LTCG — Debt / Other', stcg_other: 'STCG — Debt / Other',
  business: 'Business Income', profession: 'Professional Income', presumptive: 'Presumptive Income',
}

export default function TotalIncomePage() {
  const { salary, otherIncome } = useAppStore()
  ...
  // Build other income breakdown
  const otherItems: { label: string; value: number; sub?: string }[] = []
  if (otherIncome?.selected && otherIncome?.values) {
    for (const key of otherIncome.selected) {
      if (key === 'gift_rel') continue // exempt
      if (key === 'house_prop') {
        const rent = n(otherIncome.values['annualRentReceived'])
        const muni = n(otherIncome.values['municipalTaxPaid'])
        const netHP = Math.max(0, (rent - muni) * 0.7)
        if (netHP > 0) otherItems.push({ label: 'Income from House Property', value: netHP, sub: 'After 30% std deduction' })
        continue
      }
      const val = n(otherIncome.values[key])
      if (val > 0) otherItems.push({ label: INCOME_LABELS[key] || key, value: val })
    }
  }

  const totalOther = otherItems.reduce((s, i) => s + i.value, 0)
  const grossTotal = netSalary + totalOther
    ...
    <div className="fade-in">
      ...
      {!salary ? (
        <div style={{ textAlign: 'center', padding: '60px 20px', background: '#F8FAFB', borderRadius: 16 }}>
          ...
          <div style={{ fontSize: 14, color: '#5D6D7E', marginBottom: 20 }}>Start by uploading your AIS or salary slip</div>
          <Link href="/dashboard/ais" style={{ padding: '10px 24px', background: '#1A3C5E', color: '#fff', borderRadius: 9, fontWeight: 600, textDecoration: 'none', fontSize: 14 }}>
            Upload AIS →
          </Link>
        </div>
      ) : (
        <>
          ...
          <div style={{ background: 'linear-gradient(135deg, #0F2640, #1A3C5E)', borderRadius: 16, padding: '28px', marginBottom: 24, color: '#fff' }}>
            ...
            <div style={{ fontSize: 42, fontWeight: 800, color: '#FCD34D', lineHeight: 1 }}>₹{grossTotal.toLocaleString('en-IN')}</div>
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', marginTop: 8 }}>{salary.employeeName} · {salary.employerName}</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginTop: 20 }}>
              {[
                { label: 'Salary Income', value: `₹${netSalary.toLocaleString('en-IN')}` },
                { label: 'Other Income', value: totalOther > 0 ? `₹${totalOther.toLocaleString('en-IN')}` : '—' },
          ...
          <div style={{ background: '#fff', border: '1px solid #E5E9ED', borderRadius: 14, padding: '20px 24px', marginBottom: 20 }}>
            ...
            <Row label="Standard Deduction" value={-stdDeduction} sub="New Regime (₹75,000)" color="#C0392B" />
            ...
            {otherItems.length > 0 ? (
              otherItems.map((item, i) => <Row key={i} label={item.label} value={item.value} sub={item.sub} color="#1A3C5E" />)
            ) : (
              <div style={{ padding: '14px', background: '#F8FAFB', borderRadius: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 13, color: '#5D6D7E' }}>No other income added yet</span>
                <Link href="/dashboard/other-income" style={{ fontSize: 13, color: '#1A3C5E', fontWeight: 600, textDecoration: 'none' }}>+ Add Other Income →</Link>
              </div>
            )}
            {totalOther > 0 && <Row label="Total Other Income" value={totalOther} color="#1E8449" bold />}
            ...
            <div style={{ marginTop: 16, padding: '14px 0', borderTop: '2px solid #1A3C5E', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span style={{ fontSize: 22, fontWeight: 800, color: '#1A3C5E' }}>₹{grossTotal.toLocaleString('en-IN')}</span>
            </div>
          </div>
          ...
          <div style={{ background: '#E8F1FA', border: '1px solid #A8CCE8', borderRadius: 12, padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <div style={{ fontSize: 12, color: '#2E5A88', marginTop: 3 }}>Compare Old vs New regime and find your optimal tax</div>
            </div>
            <Link href="/dashboard/tax" style={{ padding: '10px 20px', background: '#1A3C5E', color: '#fff', borderRadius: 9, fontSize: 13, fontWeight: 700, textDecoration: 'none', whiteSpace: 'nowrap' }}>
              Optimise Tax →
            </Link>
          </div>
        </>
