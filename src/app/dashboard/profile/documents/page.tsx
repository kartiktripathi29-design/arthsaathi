...
export default function DocumentsPage() {
  ...
  const handleProceed = async () => {
    ...
    // Offer letter handling (item 9):
    //  • New joiner with NO slips anywhere → bootstrap the salary timeline from the offer so the
    //    Salary page has reviewable data and the tax pages can project the year.
    //  • Switcher (slips being uploaded now / already present) → stash the offer; the Salary page
    //    turns it into a mid-year job switch on load.
    if (offerData) {
      if (!hasAnySlips) {
    ...
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: 20, marginBottom: 16 }}>
        ...
        {offerFile ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: C.bg, borderRadius: 6, border: `1px solid ${C.border}` }}>
            <span style={{ fontSize: 18 }}>📄</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 500, color: C.fg }}>{offerFile.name}</div>
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        <button onClick={handleProceed} disabled={!(salaryFiles.length > 0 || aisFile || form26File || offerFile) || uploading} style={{
          flex: 1, padding: '12px', background: (salaryFiles.length > 0 || aisFile || form26File || offerFile) && !uploading ? C.primary : '#CCC',
