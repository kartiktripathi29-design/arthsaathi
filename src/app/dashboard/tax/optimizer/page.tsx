...
export default function TaxOptimizerPage() {
  ...
  // Presentation-only: the detailed calculation tables collapse by default so the headline numbers
  // lead. None of these affect any computed figure.
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '20px 0' }}>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: 20, marginBottom: 16 }}>
        ...
        <ExpRow id="otherslab" label="Other income (slab-taxed)" value={fmt(calc.slabOtherIncome)} sub="freelance · F&O · interest · other" editHref="/dashboard/profile/other-income" editLabel="Edit in Other Income"
          detail={<>Income taxed at your normal slab rate — freelance / consulting, F&amp;O / intraday, interest &amp; dividends, and any “other” income, plus debt-fund and unlisted-STCG gains that fall to slab. Open Other Income to see each source.</>} />
        {calc.specialIncome.total > 0 && (
          <ExpRow id="special" label="Capital gains / crypto (special rate)" value={fmt(calc.specialIncome.total)} sub="taxed separately — not at slab" editHref="/dashboard/profile/other-income" editLabel="Edit in Other Income"
            detail={<>Equity LTCG/STCG, unlisted-share LTCG and crypto are taxed at flat statutory rates — <strong>not</strong> your slab rate — so they’re kept out of the slab income above. See the “Capital gains &amp; crypto” breakdown below for the rate on each.</>} />
        )}
        <ExpRow id="exemptions" label="Total exemptions" value={fmt(calc.totalExemptions)} sub={`HRA ${fmt(calc.hraExempt)} · Other ${fmt(calc.otherExempts)}`} editHref="/dashboard/profile/exemptions" editLabel="Edit in Exemptions"
      ...
      {/* ── Tax summary — headline numbers for the recommended regime (item 13). All figures are read
            straight from the existing calc; nothing is recomputed here. ── */}
      {(() => {
        const rec = calc.recommendation === 'new'
        const slabTax = rec ? calc.newBreak.total : calc.oldBreak.total
        const totalTax = rec ? calc.newTotal : calc.oldTotal
        const balance = rec ? calc.newBalance : calc.oldBalance
        const refund = calc.tdsPaid > 0 && balance < 0
        const itrTip = `Suggested form: ${calc.itrForm}\n\n${(calc.itrReasons || []).map((r: string) => '• ' + r).join('\n')}\n\nITR-1 = salary + 1 house + interest, ≤ ₹50L, no capital gains beyond ₹1.25L LTCG\nITR-2 = capital gains, crypto, multiple houses, foreign assets, or income > ₹50L\nITR-3 = freelance with actual expenses, business, F&O\nITR-4 = presumptive freelance / small business (44ADA / 44AD), ≤ ₹50L`
        return (
          <div style={{ background: '#F0F9F7', border: `2px solid ${C.fg}`, borderRadius: 8, padding: 20, marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
              <h3 style={{ fontSize: 13, fontWeight: 700, color: C.fg, margin: 0, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Your tax · {rec ? 'New' : 'Old'} regime</h3>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <span style={{ fontSize: 9, fontWeight: 700, background: C.fg, color: '#fff', padding: '3px 8px', borderRadius: 3 }}>✓ RECOMMENDED</span>
                <span title={itrTip} style={{ fontSize: 11, fontWeight: 700, color: C.fg, background: '#fff', border: `1px solid ${C.fg}`, padding: '3px 8px', borderRadius: 3, cursor: 'help' }}>File {calc.itrForm} ⓘ</span>
              </div>
            </div>
            <Row label="Tax on salary / slab income" value={fmt(slabTax)} />
            {calc.specialTaxTotal > 0 && <Row label="+ Tax on capital gains / crypto" value={fmt(calc.specialTaxTotal)} sub="incl. 4% cess" />}
            <Row label="= Total tax" value={fmt(totalTax)} strong />
            {calc.tdsPaid > 0 && (
              <>
                <Row label="− TDS already deducted" value={fmt(calc.tdsPaid)} color={C.danger} />
                <Row label={refund ? '= Expected refund' : '= Balance tax payable'} value={fmt(Math.abs(balance))} strong color={refund ? '#2A7A4A' : C.fg} />
              </>
            )}
            <div style={{ marginTop: 12, padding: '10px 12px', background: '#fff', borderRadius: 6, border: '1px solid #D1E8E4' }}>
              <p style={{ fontSize: 11.5, color: C.text, margin: 0, lineHeight: 1.6 }}>
                New regime total <strong>{fmt(calc.newTotal)}</strong> · Old regime total <strong>{fmt(calc.oldTotal)}</strong> — you save <strong style={{ color: C.fg }}>{fmt(calc.savings)}</strong> by choosing the <strong>{rec ? 'New' : 'Old'}</strong> regime.
              </p>
              {calc.tdsPaid > 0 && (
                <p style={{ fontSize: 10, color: C.muted, margin: '6px 0 0', fontStyle: 'italic', lineHeight: 1.5 }}>
                  {calc.tdsSource === 'ais'
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: 20, marginBottom: 16 }}>
        <button onClick={() => setShowSlabwise(v => !v)} style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'inherit', padding: 0 }}>
          ...
          <span style={{ fontSize: 12, color: C.muted }}>{showSlabwise ? '− Hide' : '+ Show'}</span>
        </button>
        {showSlabwise && (
      ...
      <div style={{ background: C.wl, border: `1px solid ${C.wm}`, borderRadius: 8, padding: 20, marginBottom: 24 }}>
        ...
        {(() => {
          ...
          const saveItems = [
            ...
            { label: 'Home Loan Interest', limit: 200000, used: calc.sec24b, project: false, href: '/dashboard/profile/deductions' },
            { label: '80C Investments', limit: 150000, used: calc.sec80C, project: true, href: '/dashboard/profile/deductions' },
            { label: 'Health Insurance (80D)', limit: 100000, used: calc.sec80D, project: false, href: '/dashboard/profile/deductions' },
            { label: 'NPS (80CCD(1B))', limit: 50000, used: calc.nps, project: true, href: '/dashboard/profile/deductions' },
          ...
          {saveItems.map(s => {
            ...
            return (
              <Link key={s.label} href={s.href} style={{ display: 'block', padding: '10px', background: '#fff', borderRadius: 4, border: `1px solid ${C.border}`, textDecoration: 'none', color: 'inherit' }}>
