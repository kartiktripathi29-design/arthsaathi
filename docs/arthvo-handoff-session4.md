...
- **Mobile overflow sweep (320px)** — narrow-width horizontal scroll chased and fixed in order: (a) `dash-main` flex blowout (grew to content min-content) → `min-width: 0` — fixed `54cb81a`; (b) bottom tab-bar's four tabs summed ~370px > 320 → fluid tabs (`flex: 1 + min-width: 0`), full labels survive 320 (incl. "Tax breaks"/"Documents") — fixed `54cb81a`; (c) salary annual gross/net + gross/TDS `1fr 1fr` card rows (two 7-digit ₹ side-by-side) → stacked full-width on mobile via `.sal-annual` — fixed `7913df5`. (Salary timeline grid itself contained earlier — `9df8967`.) **Open:** (d) a NEW residual **~49px overflow at 369px** (NOT 320 — the page is *clean at 320* but *overflows at 369*; the opposite pattern from the card/timeline fixes). Offending element TBD, own diagnosis pending; affects common **360–375px** phone widths.
